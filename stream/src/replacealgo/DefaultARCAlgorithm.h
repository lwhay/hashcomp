...
#ifndef STREAM_DEFAULTARCALGORITHM_H
#define STREAM_DEFAULTARCALGORITHM_H

#include <cstring>
#include <vector>

template<typename IT>
class DefaultARCAlgorithm {
private:
...
public:
    DefaultARCAlgorithm(size_t capacity) : cacheSize(capacity), c(capacity), hashSize(3 * capacity), Hash(new IT[hashSize]) {
        std::memset(Hash, 0, sizeof(IT) * hashSize);
        // std::cout << cacheSize << " " << hashSize << std::endl;
    }

    ~DefaultARCAlgorithm() { delete[] Hash; }
...
#endif //STREAM_DEFAULTARCALGORITHM_H
