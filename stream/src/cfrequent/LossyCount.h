//
// Created by Michael on 10/29/2019.
//

#ifndef HASHCOMP_LOSSYCOUNT_H
#define HASHCOMP_LOSSYCOUNT_H
...
int lccountermerge(LCCounter *newcount, LCCounter *left, LCCounter *right,
                   int l, int r, int maxholder) {  // merge up two lists of counters. returns the size of the lists.
    int i, j, m;

    if (l + r > maxholder) { // a more advanced implementation would do a realloc here...
    ...
    while (i < l && j < r) { // merge two lists
        if (left[i].item == right[j].item) { // sum the counts of identical items
            ...
            j++;
        } else if (left[i].item < right[j].item) { // else take the left item, creating counts appropriately
            newcount[m].item = left[i].item;
            newcount[m].count = 0;
        ...
        else { // adjust for items which may have negative or zero counts
            newcount[m].item = -1;
            newcount[m].count = 0;
        }
    }
...
int lcdcountermerge(LCDCounter *newcount, LCDCounter *left, LCDCounter *right,
                    ...
                    int epoch) {  // merge up two lists of counters. returns the size of the lists.
    int i, j, m;
    ...
    while (i < l && j < r) { // merge two lists
        if (left[i].item == right[j].item) { // sum the counts of identical items
            ...
            while (left[i].item == right[j].item) { // merge in multiple copies of the same new item
                newcount[m].count += left[i].count;
                ++i;
            }
            ++j;
            ...
            while (left[i].item == newcount[m].item) { // merge in multiple new copies of the same item
                newcount[m].count += left[i].count;
        ...
        else { // overwrite an item that is not worth keeping
            newcount[m].item = -1;
            newcount[m].count = 0;
            newcount[m].delta = 0;
        }
...
void Heapify(LCL_type *lcl, int ptr) { // restore the heap condition in case it has been violated
    LCLCounter tmp;
...
LCLCounter *
LCL_FindItem(LCL_type *lcl, LCLitem_t item) { // find a particular item in the date structure and return a pointer to it
    LCLCounter *hashptr;
    int hashval;
...
int LCL_Size(LCL_type *lcl) { // return the size of the data structure in bytes
    return sizeof(LCL_type) + (lcl->hashsize * sizeof(int)) +
           (lcl->size * sizeof(LCLCounter));
}

LCLweight_t LCL_PointEst(LCL_type *lcl, LCLitem_t item) { // estimate the count of a particular item
    LCLCounter *i;
...
LCLweight_t
LCL_PointErr(LCL_type *lcl, LCLitem_t item) { // estimate the worst case error in the estimate of a particular item
    LCLCounter *i;
    i = LCL_FindItem(lcl, item);
    if (i)
        return (i->delta);
...
void LCL_CheckHash(LCL_type *lcl, int item, int hash) { // debugging routine to validate the hash table
    int i;
        ...
        while (hashptr) {
            ...
            if (hashptr->prev != prev) {
                printf("\n Previous violation! prev = %d, should be %d\n",
                       (uint64_t) hashptr->prev, (uint64_t) prev);
                printf("after inserting item %d with hash %d\n", item, hash);
                exit(EXIT_FAILURE);
            }
            prev = hashptr;
...
void LCL_ShowHash(LCL_type *lcl) { // debugging routine to show the hashtable
    int i;
    LCLCounter *hashptr;
    ...
    for (i = 0; i < lcl->hashsize; i++) {
        ...
        hashptr = lcl->hashtable[i];
        while (hashptr) {
            printf(" %d [h(%u) = %d, prev = %d] ---> ", (uint64_t) hashptr,
                   (unsigned int) hashptr->item,
                   hashptr->hash,
                   (uint64_t) hashptr->prev);
            hashptr = hashptr->next;
        }
        printf(" *** \n");
    }
...
void LCL_ShowHeap(LCL_type *lcl) { // debugging routine to show the heap
    int i, j;
