            ...
            for (int i = work->tid * total_count / thread_number;
                ...
                switch (static_cast<int>(runs[i]->getOp())) {
                    ...
                    case 3: {
                        ...
                        std::sprintf(key, "%s%llu", runs[i]->getKey(), evenRound / 2);
                        std::sprintf(value, "%s%llu", runs[i]->getVal(), evenRound / 2);
                        if (evenRound % 2 == 0) {
#if WITH_STRING
                            ret = store->Insert(Slice(string(key)), Slice(string(value)));
#else
                            ret = store->Insert(Slice(key), Slice(value));
#endif
                        } else {
#if WITH_STRING
                            bool ret = store->Delete(Slice(string(key)), &dummyVal);
#else
                            ret = store->Delete(Slice(key), &dummyVal);
#endif
                        }
                        if (ret) mhit++;
                        else mfail++;
