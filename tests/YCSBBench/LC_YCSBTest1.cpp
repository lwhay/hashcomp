...
#include <stdlib.h>
#include "tracer.h"
#include "ichash/LockFreeCuckoo1.h"
...
//        std::cout << store->Search(loads[i]->getKey()) << ":" << loads[i]->getVal() << std::endl;
        assert(std::strcmp(store->Search(loads[i]->getKey(), loads[i]->keyLength())->value, loads[i]->getVal()) == 0);
//        cout << i << endl;
#endif
    }
        ...
        while (stopMeasure.load(memory_order_relaxed) == 0) {
            for (int i = work->tid * total_count / thread_number;
                ...
                switch (static_cast<int>(runs[i]->getOp())) {
                    case 0: {
                        Entry *ret = store->Search(runs[i]->getKey(), runs[i]->keyLength());
                        rhit++;
                        break;
                    }
                    case 1: {
...
int main(int argc, char **argv) {
    ...
    cout << " threads: " << thread_number << " range: " << key_range << " count: " << total_count << " timer: "
         ...
         << readPercentage << " hash size: " << root_capacity << endl;
