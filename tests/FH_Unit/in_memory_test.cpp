...
TEST(InMemFaster, UpsertRead_Concurrent) {
    ...
    auto upsert_worker = [](FasterKv<Key, Value, FASTER::io::NullDisk> *store_,
                            size_t thread_idx) {
        store_->StartSession();
    ...
    auto read_worker = [](FasterKv<Key, Value, FASTER::io::NullDisk> *store_,
...
TEST(InMemFaster, Rmw) {
    ...
    FasterKv<Key, Value, FASTER::io::NullDisk> store{256, 1073741824, ""};
...
TEST(InMemFaster, Rmw_ResizeValue_Concurrent) {
    ...
    public:
        ...
        RmwContext(uint64_t key, int8_t incr, uint32_t length)
                : key_{key}, incr_{incr}, length_{length} {
        }

        /// Copy (and deep-copy) constructor.
        RmwContext(const RmwContext &other)
                : key_{other.key_}, incr_{other.incr_}, length_{other.length_} {
        }
...
TEST(InMemFaster, Rmw_GrowString_Concurrent) {
    ...
    public:
        ...
        RmwContext(uint64_t key, char letter)
                : key_{key}, letter_{letter} {
        }

        /// Copy (and deep-copy) constructor.
        RmwContext(const RmwContext &other)
                : key_{other.key_}, letter_{other.letter_} {
        }
    ...
    auto rmw_worker = [](FasterKv<Key, Value, FASTER::io::NullDisk> *store_, char start_letter) {
        store_->StartSession();
...
TEST(InMemFaster, GrowHashTable) {
    ...
    auto rmw_worker0 = [](FasterKv<Key, Value, FASTER::io::NullDisk> *store_,
                          int64_t incr) {
