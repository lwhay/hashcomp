...
#endif
                if (hash_freent >= 5 && hash_freent <= 14)
                    ptr = (node *) deallocator->allocate(tid); // useless tid in 6-12
...
int main(int argc, char **argv) {
    ...
    readops = new uint64_t[thrd_number];
    writeops = new uint64_t[thrd_number];
    ...
    for (; t < thrd_number; t++) {
        ...
        workers.push_back(std::thread(worker, bucket, t));
#if defined(linux) && ENABLE_NUMA == 1
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
        CPU_SET(t, &cpuset);
    ...
    for (size_t t = 0; t < thrd_number; t++) {
        ...
        if (t < worker_gran) {
            ...
            readcount += readops[t];
            if (detail_print == 1)
                std::cout << "\tReader" << t << ": " << readops[t] << "\t" << runtime[t] << std::endl;
        } else {
            writetime += runtime[t];
            writecount += writeops[t];
            freeconflict += conflict[t];
            if (detail_print == 1)
                std::cout << "\tWriter" << t - worker_gran << ": " << writeops[t] << "\t" << runtime[t] << std::endl;
        }
    }
