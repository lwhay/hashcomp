...
void RandomNumberBenchmark(int thread_num, int iter) {
    ...
    auto f = [thread_num, iter](uint64_t id) -> void {
        Random<uint64_t, 0, (uint64_t) -1> r{};
 ...
 */
void LEMSimpleBenchmark(uint64_t thread_num, uint64_t op_num, uint64_t workload) {
 ...
 */
void GEMSimpleBenchmark(uint64_t thread_num, uint64_t op_num, uint64_t workload) {
 ...
 */
void GetValueAsULOrThrow(Argv &args, const std::string &key, uint64_t *value_p) {
