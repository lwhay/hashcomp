...
namespace libcuckoo {
    ...
    inline static partial_t extract_partial(uint64_t entry) {
        cdebug ASSERT(!is_kick_marked(entry),"try extract partial from an entry marked")
    ...
    inline static thread_mark_t extract_kick_thread_mark(uint64_t entry){
        cdebug ASSERT(is_kick_marked(entry),"try extract kick thread mark from an entry not marked")
    ...
    inline uint64_t mark_entry(uint64_t entry,int tid){
        cdebug ASSERT(entry != empty_entry,"should not mark empty entry")
        cdebug ASSERT(!is_kick_marked(entry),"try mark an marked antry")
    ...
    inline uint64_t con_hp_store(uint64_t entry){
        return ( entry & ptr_mask );
    }
