...
//    thread_local bool thread_initialized;
    ...
    public:
        ...
        bool inquiry_need_redo(){
            return redo_flags[cuckoo_tid * ALIGN_RATIO_BOOL].load(std::memory_order_acquire);
        }

        void clean_redo_flag(){
            redo_flags[cuckoo_tid * ALIGN_RATIO_BOOL].store(false,std::memory_order_release);
        }
        ...
        void set_redo_if_hazard(size_type hash){
            ...
            for(int i = 0; i < running_thread_num; i++){
                ...
                size_type store_record =  manager[(i * REGISTER_CAPACITY + reg_index) * ALIGN_RATIO].load(std::memory_order_acquire);
                if(is_handled(store_record) && equal_hash(store_record,hash)){
                    record llog.kick_send_redo_signal++;
                    redo_flags[i * ALIGN_RATIO_BOOL].store(true,std::memory_order_release);
        ...
        void register_hash(size_type hash) {
            int reg_index = hash % REGISTER_CAPACITY;
            //SOFTWARE_BARRIER;
            manager[(cuckoo_tid * REGISTER_CAPACITY + reg_index )* ALIGN_RATIO].store(con_store_record(hash),std::memory_order_release);
            //SOFTWARE_BARRIER;
        }
        ...
        inline bool equal_hash(size_type store_record,size_type hash){
            cdebug ASSERT(is_handled(store_record),"compare record not be handled");
