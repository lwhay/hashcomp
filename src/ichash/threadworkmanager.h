    ...
    public:
        ...
        void set_redo_if_hazard(size_type hash){
            ...
            for(int i = 0; i < running_thread_num; i++){
                ...
                size_type store_record =  manager[(i * REGISTER_CAPACITY + reg_index) * ALIGN_RATIO].load(std::memory_order_seq_cst);
        ...
        void register_hash(size_type hash) {
            int reg_index = hash % REGISTER_CAPACITY;
            SOFTWARE_BARRIER;
            manager[(cuckoo_tid * REGISTER_CAPACITY + reg_index )* ALIGN_RATIO].store(con_store_record(hash),std::memory_order_relaxed);
            SOFTWARE_BARRIER;
        }
