...
//                           (slot_per_bucket() - 1));
        ...
        int cuckoopath_search(const size_type hp, CuckooRecords &cuckoo_path,
...

            {
                ...
                cdebug ASSERT(ptr!= nullptr,"ptr null !")
        ...
        void run_cuckoo(TwoBuckets &b) {
            ...
            while (true) {
                ...
                loop_count ++;
                cdebug ASSERT(loop_count < 1000000,"MAYBE DEAD LOOP");
        ...
        table_position Set_find(hash_value hv, TwoBuckets &b, char *key, size_type key_len) {
            ...
            while (true){
                if (!try_find_insert_bucket(b.i1, res1, hv.partial, key, key_len,entry)) {
                    return table_position{b.i1, res1,entry,
                                          key_hit};
                }
                if (!try_find_insert_bucket(b.i2, res2, hv.partial, key, key_len,entry)) {
                    return table_position{b.i2, res2,entry,
                                          key_hit};
                }
                if (res1 != -1) {
        ...
        void help_cuckoo_move(table_position pos){
            cdebug ASSERT(is_kick_marked(pos.entry),"helper parameter false ")
        ...
        void check_anapshoot(uint64_t * entry_snapshoot,uint64_t entry, bool & self_exist ,int & self_position,int & first_position){
                ...
                if(count == 1){
                    ...
                    if(!self_exist){
                        ...
                        if(is_kick_marked(entry_snapshoot[self_position])){
                            ...
                            help_cuckoo_move({ind,self_position % SLOT_PER_BUCKET,entry_snapshoot[self_position],kick_parameter});
                            continue;
                        }
    ...
    public:
        ...
        void init_thread(int tid){ cuckoo_tid = tid; bc.init_thread(tid); }
        ...
        void get_key_position_info(std::vector<double> & kpv,size_type item_num){bc.get_key_position_info(kpv,item_num);}
        ...
        bool Set(char * key, size_t key_len, char* value , size_t value_len){
            ...
            while (true){
                ...
                if(pos.status == key_miss_with_empty_slot){
                    ...
                    cdebug ASSERT(pos.status == key_hit,"pos.status false");
