...
#define SCXAndEnterQuiescentState scx
    ...
    bool updateInsert(const int, const K &key, const V &val, const bool onlyIfAbsent, V *result,
                      bool *shouldRebalance); // last 2 args are output args
...
public:
    ...
    long long debugKeySum() {
        return debugKeySum((Node<K, V> *) (
                ((Node<K, V> *) root->left.load(memory_order_relaxed))->left.load(memory_order_relaxed)));
    }
