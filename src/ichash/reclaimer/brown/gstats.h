...
public:
    ...
    gstats_output_item(gstats_enum_output_method method, gstats_enum_aggregation_function func,
            ...
            : method(method), func(func), granularity(granularity),
...
public:
    gstats_t(const int num_processes)
        ...
        VERBOSE std::cout << "parallel stat intialization: spawning " << std::thread::hardware_concurrency()
                          << " threads" << std::endl;
...
//                while (!start) { __sync_synchronize(); }
                ...
                int end_ix = (thread_id == std::thread::hardware_concurrency() - 1) ? this->num_indices[id] :
        ...
        start = true;
        __sync_synchronize();
        VERBOSE std::cout << "parallel compute histogram: joining " << std::thread::hardware_concurrency() << " threads"
                          << std::endl;
...
//            std::cout<<"start compute_before_printing()..."<<std::endl;
        ...
        VERBOSE std::cout << "parallel stats compute before printing: spawning " << (3 * num_stats) << " threads"
                          << std::endl;
        std::thread threads[3 * num_stats];
        for (int id = 0; id < num_stats; ++id) {
            if (this->data_types[id] == LONG_LONG) {
                threads[3 * id + 0] = std::thread([this, id]() {
        ...
        VERBOSE std::cout << "parallel stats compute before printing: joining " << (3 * num_stats) << " threads"
                          << std::endl;
...
//                    return compute_stat_metrics_by_thread<double>(id);
    ...
    void print_stat(const gstats_stat_id id, gstats_output_item &output_item) {
        ...
        if (output_item.func == NONE) {
            if (output_item.granularity != FULL_DATA)
                setbench_error("must use aggregation granularity FULL_DATA when using aggregation function NONE");
        }
        if (output_item.granularity == FULL_DATA) {
            if (output_item.func != NONE)
                setbench_error("must use aggregation function NONE when using aggregation granularity FULL_DATA");
        }
            ...
            case PRINT_RAW: {
                switch (output_item.func) {
                    ...
                    case NONE: GSTATS_PRINT_AGG(granularity_str, NONE, id, metrics, num_metrics);
                        break;
                    default:
                        setbench_error("should not reach here");
                }
            }
                break;
            case PRINT_HISTOGRAM_LOG: {
                if (output_item.granularity == TOTAL)
                    setbench_error(
    ...
    void print_all() {
        for (auto it = output_config.begin(); it != output_config.end(); it++) {
            GSTATS_USE_TEMPLATE(it->first, print_stat, it->first
                    GSTATS_COMMA
                    it->second);
        }
    }
