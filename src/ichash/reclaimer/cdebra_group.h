//
// Created by czl on 2021/5/15.
//

#ifndef CZL_CUCKOO_CDEBRA_GROUP_H
#define CZL_CUCKOO_CDEBRA_GROUP_H
...
public:
    ...
    Reclaimer_debra_group(int thread_num);
...
Reclaimer_debra_group::Reclaimer_debra_group(int thread_num) : NUM_PROCESSES(thread_num) {
    alloc = new Alloc;
    epoch = 0;
    for (int tid = 0; tid < NUM_PROCESSES; ++tid) {
        threadData[tid].Q = true;
...
void Reclaimer_debra_group::initThread(int tid) {
    for (int i = 0; i < NUMBER_OF_EPOCH_BAGS; ++i) {
...
storeType *Reclaimer_debra_group::allocate(int tid, uint64_t len) {
    return static_cast<storeType *>(alloc->allocate(len));
}

bool Reclaimer_debra_group::deallocate(int tid, storeType *ptr) {
...
void Reclaimer_debra_group::rotate_epoch_bag(int tid) {
...
bool Reclaimer_debra_group::startOp(int tid) {
    ...
    long readEpoch = group_id == 0 ? epoch : (threadData[group_id].announcedEpoch.load(std::memory_order_relaxed) & ~1l );
    ...
    SOFTWARE_BARRIER;
    threadData[tid].Q.store(false,std::memory_order_relaxed); // note: this must be written, regardless of whether the announced epochs are the same, because the quiescent bit will vary
    ...
    if (++threadData[tid].opsSinceRead == MIN_OPS_BEFORE_READ) {
        ...
        bool otherq = threadData[otherTid].Q.load(std::memory_order_relaxed);
        if (BITS_EPOCH(otherAnnounce) == readEpoch || otherq ) {
...
void Reclaimer_debra_group::endOp(int tid) {
    threadData[tid].Q.store(true,
                                         std::memory_order_relaxed);
}

void Reclaimer_debra_group::retire(int tid, storeType *ptr) {
    threadData[tid].currentBag->add(ptr);
}

#endif //CZL_CUCKOO_CDEBRA_GROUP_H
