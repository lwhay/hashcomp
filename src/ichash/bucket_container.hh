...
#include "cuckoohash_config.hh"
#include "item.h"
#include "reclaimer/cdebra.h"
#include "entry_def.h"
#include "threadworkmanager.h"
#include "reclaimer/interface.h"
...
namespace libcuckoo {
    ...
    public:
        ...
        void destory_bc() {
            ...
            cdebug ASSERT(!still_have_item, "bucket still have item!");
            delete[]buckets_;
        }
...
//        const Bucket_container &operator[](size_type i) const { return buckets_[i]; }
        ...
        void endOp(){rc.clear(cuckoo_tid);rc.endOp(cuckoo_tid);}
        ...
        Item *allocate_item(char *key, size_t key_len, char *value, size_t value_len) {
            ...
            cdebug ASSERT(p != nullptr, "malloc failure");
            p->key_len = key_len;
        ...
        inline size_type read_from_bucket_slot(size_type ind, size_type slot) {
            return rc.read(cuckoo_tid,buckets_[ind].values_[slot]);
        }

        inline size_type read_from_bucket_slot_drt(size_type ind, size_type slot) {
        ...
        bool unique_in_two_buckets(size_type i1,size_type slot_ind,size_type i2){
            ...
            uint64_t check_entry = read_from_bucket_slot_drt(i1,slot_ind);
            ...
            for(int i = 0 ; i < SLOT_PER_BUCKET;i++){
                ...
                uint64_t com_entry = read_from_bucket_slot_drt(i1,i);
                ...
                if(com_par != check_par){
                    ...
                    if(str_equal_to()(ITEM_KEY(check_ptr),ITEM_KEY_LEN(check_ptr),ITEM_KEY(com_ptr),ITEM_KEY_LEN(com_ptr)))
                        return false;
                }
            }
            ...
            for(int i = 0 ; i < SLOT_PER_BUCKET;i++){
                ...
                uint64_t com_entry = read_from_bucket_slot_drt(i2,i);
                if(com_entry == (uint64_t) nullptr) continue;
                partial_t com_par = extract_partial(com_entry);
                Item * com_ptr = extract_ptr(com_entry);
                if(com_par != check_par){
        ...
        void dump_two_buckets(size_type b1,size_type b2){
            ...
            for(int i = 0; i < SLOT_PER_BUCKET; i++){
                cout<<"slot "<<i<<"\t";
                uint64_t entry = read_from_bucket_slot_drt(b1,i);
            ...
            for(int i = 0; i < SLOT_PER_BUCKET; i++){
                cout<<"slot "<<i<<"\t";
                uint64_t entry = read_from_bucket_slot_drt(b2,i);
                if(entry == empty_entry){
                    cout<<"0"<<endl;
                }else{
                    Item * ptr = extract_ptr(entry);
        ...
        uint64_t get_item_num_and_check() {
            ...
            for (size_t i = 0; i < get_size(); i++) {
                ...
                for (size_t j = 0; j < SLOT_PER_BUCKET; j++) {
                    ...
                    cdebug ASSERT(!is_kick_marked(entry),"ERROR!!!There are still marked entries in the table!");
                    if (entry != empty_entry) {
                        count++;
                        cdebug {
        ...
        void get_key_position_info(std::vector<double> &kpv,size_type item_num) {
            table_anaz_mtx.lock();
            cdebug ASSERT(kpv.size() == SLOT_PER_BUCKET, "key_position_info length error");
