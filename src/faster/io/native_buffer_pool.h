...
public:
    /// Default constructor.
    SectorAlignedMemory()
            : buffer_{nullptr}, valid_offset{0}, required_bytes{0}, available_bytes{0}, level_{0},
              pool_{nullptr} {
    }

    SectorAlignedMemory(uint8_t *buffer, uint32_t level, NativeSectorAlignedBufferPool *pool)
            : buffer_{buffer}, valid_offset{0}, required_bytes{0}, available_bytes{0}, level_{level},
              pool_{pool} {
    }
...
inline SectorAlignedMemory NativeSectorAlignedBufferPool::Get(uint32_t numRecords) {
    ...
    if (queue_[level].try_pop(buffer)) {
        ...
        uint8_t *buffer = reinterpret_cast<uint8_t *>(aligned_alloc(sector_size_, sector_size_ * (1 << level)));
        return SectorAlignedMemory{buffer, level, this};
    }
}
