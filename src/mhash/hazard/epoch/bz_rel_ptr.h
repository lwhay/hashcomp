...
public:
    ...
    rel_ptr(const T *abs_ptr) : off((unsigned char *) abs_ptr - base_address) {}

    explicit rel_ptr(uint64_t rel_addr) : off(rel_addr) {}

    template<typename U>
    rel_ptr(rel_ptr<U> rptr) : off((unsigned char *) rptr.abs() - base_address) {}

    rel_ptr(PMEMoid oid) : off((unsigned char *) pmemobj_direct(oid) - base_address) {}
...
template<typename T>
unsigned char *rel_ptr<T>::base_address(nullptr);
template<typename T>
unsigned char rel_ptr<T>::base_oid(OID_NULL);
