...
private:
    ...
    public:
         ...
         * Constructor
         */
        ChunkHeader(uint32_t p_ref_count, uint32_t p_offset) : ref_count{p_ref_count}, offset{p_offset} {}
    ...
    public:
         ...
         * Constructor
         */
        Chunk(size_t sz) : header{ChunkHeader{0, 0}}, next_p{nullptr} {
            data = new char[sz];
            assert(data != nullptr);
        ...
        void *Allocate(size_t sz) {
            ...
            while (1) {
                ...
                ChunkHeader new_header{expected_header.ref_count + 1, new_offset};

                // If success this will exchange into expected_header
                bool ret = header.compare_exchange_strong(expected_header, new_header);
    ...
    Chunk *AllocateChunk(size_t sz) {
        ...
        bool ret = appending_tail_p.load()->next_p.compare_exchange_strong(expected_chunk, chunk_p);
        if (ret == false) {
            delete chunk_p;
...
public:
    ...
    void Free(void *p) {
        ...
        while (1) {
            ...
            bool ret = mem_p->chunk_p->header.compare_exchange_strong(header, {header.ref_count - 1, header.offset});
            if (ret == true) {
     ...
     * Constructor
     */
    VarLenPool(size_t p_chunk_size) : chunk_size{p_chunk_size} {
