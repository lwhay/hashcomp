...
private:
    ...
    public:
         ...
         * the CAS process
         */
        GarbageNode(GarbageType *p_garbage_p, CounterType p_deleted_epoch) : deleted_epoch{p_deleted_epoch},
...
private:
    ...
    ElementType *AlignToCacheLine(void *p) {
        ...
        ElementType *q = reinterpret_cast<ElementType *>((reinterpret_cast<uint64_t>(p) + (CACHE_LINE_SIZE - 1)) &
                                                         cache_line_mask);
...
public:
     ...
     */
    LocalWriteEM(uint64_t p_core_num) : core_num{p_core_num} {
