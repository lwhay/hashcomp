            ...
            while (cix > EBRT_ROOT) {
                ...
                if (startingOp) {
                    while (true) {
                        ...
                        if (pix == EBRT_ROOT)
                            newval += EPOCH_INCREMENT;                                                // TODO: detect epoch overflow
                        ...
                        if (QUIESCENT(pval)) {                                  // TODO: unlikely ?
                            if (BITS_EPOCH(pval) < newval) {                    // TODO: unlikely
                                ...
                                auto retval = CASV(&p->v, pval, pval +
                                                                SEQUENCE_INCREMENT);                               // TODO: detect sequence overflow
                                if (retval == pval) break; // continue at parent
                                pval = retval;             // retry at this node
                                SOFTWARE_BARRIER;
                                sval = nodes[EBRT_SIBLING(cix)].v; // reread sibling value AFTER parent
        ...
        void debugPrint() {
            ...
            for (; rowsize <= numThreadsPow2; ++row, rowsize <<= 1) {
                ...
                for (int i = 0; i < rowsize; ++i) {
                    auto v = nodes[ix].v;
                    std::cout << " " << (BITS_EPOCH(v) >> 32LL) << "," << (GET_SEQUENCE(v)) << ","
                              << ((int) QUIESCENT(v));
                    ++ix;
                }
                std::cout << std::endl;
...
public:
    ...
    struct rebind {
        typedef reclaimer_ebr_tree_q<_Tp1, Pool> other;
    };
    template<typename _Tp1, typename _Tp2>
    struct rebind2 {
        typedef reclaimer_ebr_tree_q<_Tp1, _Tp2> other;
    };
...
//        }
    ...
    reclaimer_ebr_tree_q(const int numProcesses, Pool *_pool, debugInfo *const _debug,
                         RecoveryMgr<void *> *const _recoveryMgr = NULL)
            : reclaimer_interface<T, Pool>(numProcesses, _pool, _debug, _recoveryMgr) {
        VERBOSE std::cout << "constructor reclaimer_ebr_tree_q helping=" << this->shouldHelp()
