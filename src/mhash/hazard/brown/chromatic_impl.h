...
Chromatic<K, V, Compare, MasterRecordMgr>::Chromatic(const K &_NO_KEY,
    ...
    VERBOSE
    DEBUG COUTATOMIC("constructor chromatic" << endl);
    const int tid = 0;
    ...
    if (key == l->key) {
        ...
        *result = l->value;
        ...
        if (l->key == NO_KEY || cmp(key, l->key)) {
            assert(recordmgr->isProtected(tid, l));
            /*newInternal = */
            initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), l->key, l->value, newWeight,
                           GET_ALLOCATED_NODE_PTR(tid, 0), GET_ALLOCATED_NODE_PTR(tid, 1));
        } else {
            /*newInternal = */
            initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), key, val, newWeight, GET_ALLOCATED_NODE_PTR(tid, 1),
        ...
        *shouldRebalance = (count > N);
        ...
        if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_INS, nodes, llxResults,
                                      (l == pleft ? &p->left : &p->right), GET_ALLOCATED_NODE_PTR(tid, 2))) {
...
bool Chromatic<K, V, Compare, MasterRecordMgr>::updateRebalancingStep(
    ...
    assert(recordmgr->isProtected(tid, l));
    while ((Node <K, V> *) l->left.load(memory_order_relaxed) != NULL && l->weight <= 1 &&
    ...
    Node <K, V> *uXRL, *uXRR;
    void *iuXXL;
    Node <K, V> *uXXLL, *uXXLR;
    void *iuXXR;
    Node <K, V> *uXXRL, *uXXRR;
    void *iuXXLL;
    Node <K, V> *uXXLLL, *uXXLLR;
    void *iuXXLR;
    Node <K, V> *uXXLRL, *uXXLRR;
    void *iuXXRL;
    Node <K, V> *uXXRLL, *uXXRLR;
    void *iuXXRR;
    Node <K, V> *uXXRRL, *uXXRRR;
    void *iuXXLRL;
    Node <K, V> *uXXLRLL, *uXXLRLR;
    void *iuXXLRR;
    Node <K, V> *uXXLRRL, *uXXLRRR;
    void *iuXXRLL;
    Node <K, V> *uXXRLLL, *uXXRLLR;
    void *iuXXRLR;
    Node <K, V> *uXXRLRL, *uXXRLRR;
    ...
    if (uXXX->weight > 1) {
        if (uXXXleft) {
            ...
            if (uXXR->weight == 0) {
                if (uXX->weight == 0) {
                    if (uXXleft) {
                        IF_FAIL_TO_PROTECT_NODE(info, tid, uXR, &uX->right,
                                                &uX->marked) return false; // return and retry
                        if ((iuXR = LLX(tid, uXR, &uXRL, &uXRR)) == NULL) return false; // return and retry

                        if (uXR->weight == 0) {
                            void *llxResults[] = {iu, iuX, iuXX, iuXR};
                            Node <K, V> *nodes[] = {u, uX, uXX, uXR};
                            return doBlk(tid, nodes, llxResults, uXleft);
                        } else { // assert: uXR->weight > 0
                            void *llxResults[] = {iu, iuX, iuXX, iuXXR};
                            Node <K, V> *nodes[] = {u, uX, uXX, uXXR};
                            return doRb2(tid, nodes, llxResults, uXleft);
                        }
                    } else { // assert: uXX == uXR
                        IF_FAIL_TO_PROTECT_NODE(info, tid, uXL, &uX->left,
                                                &uX->marked) return false; // return and retry
                        if ((iuXL = LLX(tid, uXL, &uXLL, &uXLR)) == NULL) return false; // return and retry

                        if (uXL->weight == 0) {
                            void *llxResults[] = {iu, iuX, iuXL, iuXX};
                            Node <K, V> *nodes[] = {u, uX, uXL, uXX};
                            return doBlk(tid, nodes, llxResults, uXleft);
                        } else {
                            void *llxResults[] = {iu, iuX, iuXX};
                            Node <K, V> *nodes[] = {u, uX, uXX};
                            return doRb1Sym(tid, nodes, llxResults, uXleft);
                        }
                    }
                } else { // assert: uXX->weight > 0
                    IF_FAIL_TO_PROTECT_NODE(info, tid, uXXRL, &uXXR->left,
                                            &uXXR->marked) return false; // return and retry
                    if ((iuXXRL = LLX(tid, uXXRL, &uXXRLL, &uXXRLR)) == NULL) return false; // return and retry

                    if (uXXRL->weight > 1) {
                        void *llxResults[] = {iuX, iuXX, iuXXL, iuXXR, iuXXRL};
                        Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXR, uXXRL};
                        return doW1(tid, nodes, llxResults, uXXleft);
                    } else if (uXXRL->weight == 0) {
                        void *llxResults[] = {iuX, iuXX, iuXXR, iuXXRL};
                        Node <K, V> *nodes[] = {uX, uXX, uXXR, uXXRL};
                        ...
                        IF_FAIL_TO_PROTECT_NODE(info, tid, uXXRLR, &uXXRL->right,
                                                &uXXRL->marked) return false; // return and retry
                        if ((iuXXRLR = LLX(tid, uXXRLR, &uXXRLRL, &uXXRLRR)) == NULL) return false; // return and retry

                        if (uXXRLR->weight == 0) {
                            void *llxResults[] = {iuX, iuXX, iuXXL, iuXXR, iuXXRL, iuXXRLR};
                            Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXR, uXXRL, uXXRLR};
                            ...
                            IF_FAIL_TO_PROTECT_NODE(info, tid, uXXRLL, &uXXRL->left,
                                                    &uXXRL->marked) return false; // return and retry
                            if ((iuXXRLL = LLX(tid, uXXRLL, &uXXRLLL, &uXXRLLR)) == NULL)
                                return false; // return and retry

                            if (uXXRLL->weight == 0) {
                                void *llxResults[] = {iuX, iuXX, iuXXL, iuXXR, iuXXRL, iuXXRLL};
                                Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXR, uXXRL, uXXRLL};
                ...
                if (uXXRR->weight == 0) {
                    void *llxResults[] = {iuX, iuXX, iuXXL, iuXXR, iuXXRR};
                    Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXR, uXXRR};
                    ...
                    void *llxResults[] = {iuX, iuXX, iuXXL, iuXXR};
                    Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXR};
            ...
            if (uXXL->weight == 0) {
                if (uXX->weight == 0) {
                    if (!uXXleft) {
                        ...
                        if (uXL->weight == 0) {
                            ...
                            void *llxResults[] = {iu, iuX, iuXX, iuXXL};
                    ...
                    IF_FAIL_TO_PROTECT_NODE(info, tid, uXXLR, &uXXL->right,
                                            &uXXL->marked) return false; // return and retry
                    if ((iuXXLR = LLX(tid, uXXLR, &uXXLRL, &uXXLRR)) == NULL) return false; // return and retry

                    if (uXXLR->weight > 1) {
                        void *llxResults[] = {iuX, iuXX, iuXXL, iuXXR, iuXXLR};
                        Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXR, uXXLR};
                        return doW1Sym(tid, nodes, llxResults, uXXleft);
                    } else if (uXXLR->weight == 0) {
                        void *llxResults[] = {iuX, iuXX, iuXXL, iuXXLR};
                        Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXLR};
                        ...
                        IF_FAIL_TO_PROTECT_NODE(info, tid, uXXLRL, &uXXLR->left,
                                                &uXXLR->marked) return false; // return and retry
                        if ((iuXXLRL = LLX(tid, uXXLRL, &uXXLRLL, &uXXLRLR)) == NULL) return false; // return and retry

                        if (uXXLRL->weight == 0) {
                            void *llxResults[] = {iuX, iuXX, iuXXL, iuXXR, iuXXLR, iuXXLRL};
                            Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXR, uXXLR, uXXLRL};
                            ...
                            IF_FAIL_TO_PROTECT_NODE(info, tid, uXXLRR, &uXXLR->right,
                                                    &uXXLR->marked) return false; // return and retry
                            if ((iuXXLRR = LLX(tid, uXXLRR, &uXXLRRL, &uXXLRRR)) == NULL)
                                return false; // return and retry

                            if (uXXLRR->weight == 0) {
                                void *llxResults[] = {iuX, iuXX, iuXXL, iuXXR, iuXXLR, iuXXLRR};
                                Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXR, uXXLR, uXXLRR};
                ...
                if (uXXLL->weight == 0) {
                    void *llxResults[] = {iuX, iuXX, iuXXL, iuXXR, iuXXLL};
                    Node <K, V> *nodes[] = {uX, uXX, uXXL, uXXR, uXXLL};
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doBlk(const int tid, Node <K, V> **nodes, void **llxResults,
                                                      bool fieldIsLeft) {
    Node <K, V> *nodeXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[2]->key, nodes[2]->value, 1,
                                         (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                         (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[3]->key, nodes[3]->value, 1,
                                         (Node <K, V> *) nodes[3]->left.load(memory_order_relaxed),
                                         (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    int weight = (IS_SENTINEL(nodes[1], nodes[0]) ? 1 : nodes[1]->weight - 1); // root of old subtree is a sentinel
    Node <K, V> *nodeX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[1]->key, nodes[1]->value, weight,
                                        nodeXL, nodeXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_BLK, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeX)) {
        return true;
    } else {
        return false;
    }
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doRb1(const int tid, Node <K, V> **nodes, void **llxResults,
                                                      bool fieldIsLeft) {
    Node <K, V> *nodeXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 0,
                                         (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed),
                                         (Node <K, V> *) nodes[1]->right.load(memory_order_relaxed));
    int weight = nodes[1]->weight;
    Node <K, V> *nodeX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[2]->key, nodes[2]->value, weight,
                                        (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed), nodeXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_RB1, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doRb2(const int tid, Node <K, V> **nodes, void **llxResults,
                                                      bool fieldIsLeft) {
    Node <K, V> *nodeXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[2]->key, nodes[2]->value, 0,
                                         (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                         (Node <K, V> *) nodes[3]->left.load(memory_order_relaxed));
    Node <K, V> *nodeXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[1]->key, nodes[1]->value, 0,
                                         (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed),
                                         (Node <K, V> *) nodes[1]->right.load(memory_order_relaxed));
    int weight = nodes[1]->weight;
    Node <K, V> *nodeX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[3]->key, nodes[3]->value, weight,
                                        nodeXL, nodeXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_RB2, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doPush(const int tid, Node <K, V> **nodes, void **llxResults,
                                                       bool fieldIsLeft) {
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[2]->key, nodes[2]->value,
                                          nodes[2]->weight - 1,
                                          (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                          (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[3]->key, nodes[3]->value, 0,
                                          (Node <K, V> *) nodes[3]->left.load(memory_order_relaxed),
                                          (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    int weight = (IS_SENTINEL(nodes[1], nodes[0]) ? 1 : nodes[1]->weight + 1); // root of old subtree is a sentinel
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[1]->key, nodes[1]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_PUSH, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
    }
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW1(const int tid, Node <K, V> **nodes, void **llxResults,
                                                     bool fieldIsLeft) {
    Node <K, V> *nodeXXLL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[2]->key, nodes[2]->value,
                                           nodes[2]->weight - 1,
                                           (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXLR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[4]->key, nodes[4]->value,
                                           nodes[4]->weight - 1,
                                           (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[4]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 1,
                                          nodeXXLL, nodeXXLR);
    int weight = nodes[1]->weight;
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[3]->key, nodes[3]->value, weight,
                                         nodeXXL, (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W1, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW2(const int tid, Node <K, V> **nodes, void **llxResults,
                                                     bool fieldIsLeft) {
    Node <K, V> *nodeXXLL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[2]->key, nodes[2]->value,
                                           nodes[2]->weight - 1,
                                           (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXLR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[4]->key, nodes[4]->value, 0,
                                           (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[4]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 1,
                                          nodeXXLL, nodeXXLR);
    ...
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[3]->key, nodes[3]->value, weight,
                                         nodeXXL, (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W2, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW3(const int tid, Node <K, V> **nodes, void **llxResults,
                                                     bool fieldIsLeft) {
    Node <K, V> *nodeXXLLL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 4), nodes[2]->key, nodes[2]->value,
                                            nodes[2]->weight - 1,
                                            (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                            (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXLL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[1]->key, nodes[1]->value, 1,
                                           nodeXXLLL, (Node <K, V> *) nodes[5]->left.load(memory_order_relaxed));
    Node <K, V> *nodeXXLR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[4]->key, nodes[4]->value, 1,
                                           (Node <K, V> *) nodes[5]->right.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[4]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[5]->key, nodes[5]->value, 0,
                                          nodeXXLL, nodeXXLR);
    int weight = nodes[1]->weight;
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[3]->key, nodes[3]->value, weight,
                                         nodeXXL, (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W3, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW4(const int tid, Node <K, V> **nodes, void **llxResults,
                                                     bool fieldIsLeft) {
    Node <K, V> *nodeXXLL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[2]->key, nodes[2]->value,
                                           nodes[2]->weight - 1,
                                           (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 1,
                                          nodeXXLL, (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed));
    Node <K, V> *nodeXXRL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 4), nodes[5]->key, nodes[5]->value, 1,
                                           (Node <K, V> *) nodes[5]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[5]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[3]->key, nodes[3]->value, 0,
                                          nodeXXRL, (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    int weight = nodes[1]->weight;
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[4]->key, nodes[4]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W4, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW5(const int tid, Node <K, V> **nodes, void **llxResults,
                                                     bool fieldIsLeft) {
    Node <K, V> *nodeXXLL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[2]->key, nodes[2]->value,
                                           nodes[2]->weight - 1,
                                           (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 1,
                                          nodeXXLL, (Node <K, V> *) nodes[3]->left.load(memory_order_relaxed));
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[4]->key, nodes[4]->value, 1,
                                          (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed),
                                          (Node <K, V> *) nodes[4]->right.load(memory_order_relaxed));
    int weight = nodes[1]->weight;
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[3]->key, nodes[3]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W5, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW6(const int tid, Node <K, V> **nodes, void **llxResults,
                                                     bool fieldIsLeft) {
    Node <K, V> *nodeXXLL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[2]->key, nodes[2]->value,
                                           nodes[2]->weight - 1,
                                           (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
    ...
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 1,
                                          nodeXXLL, (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed));
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[3]->key, nodes[3]->value, 1,
                                          (Node <K, V> *) nodes[4]->right.load(memory_order_relaxed),
                                          (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    int weight = nodes[1]->weight;
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[4]->key, nodes[4]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W6, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW7(const int tid, Node <K, V> **nodes, void **llxResults,
                                                     bool fieldIsLeft) {
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[2]->key, nodes[2]->value,
                                          nodes[2]->weight - 1,
                                          (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                          (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[3]->key, nodes[3]->value,
                                          nodes[3]->weight - 1,
                                          (Node <K, V> *) nodes[3]->left.load(memory_order_relaxed),
                                          (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    int weight = (IS_SENTINEL(nodes[1], nodes[0]) ? 1 : nodes[1]->weight + 1); // root of old subtree is a sentinel
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[1]->key, nodes[1]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W7, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doRb1Sym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                         bool fieldIsLeft) {
    Node <K, V> *nodeXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 0,
                                         (Node <K, V> *) nodes[1]->left.load(memory_order_relaxed),
                                         (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed));
    int weight = nodes[1]->weight;
    Node <K, V> *nodeX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[2]->key, nodes[2]->value, weight,
                                        nodeXL, (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_RB1SYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doRb2Sym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                         bool fieldIsLeft) {
    Node <K, V> *nodeXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 0,
                                         (Node <K, V> *) nodes[1]->left.load(memory_order_relaxed),
                                         (Node <K, V> *) nodes[3]->left.load(memory_order_relaxed));
    Node <K, V> *nodeXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[2]->key, nodes[2]->value, 0,
                                         (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed),
                                         (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    int weight = nodes[1]->weight;
    Node <K, V> *nodeX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[3]->key, nodes[3]->value, weight,
                                        nodeXL, nodeXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_RB2SYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doPushSym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                          bool fieldIsLeft) {
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[2]->key, nodes[2]->value, 0,
                                          (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                          (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[3]->key, nodes[3]->value,
                                          nodes[3]->weight - 1,
    ...
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[1]->key, nodes[1]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_PUSHSYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW1Sym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                        bool fieldIsLeft) {
    Node <K, V> *nodeXXRL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[4]->key, nodes[4]->value,
                                           nodes[4]->weight - 1,
                                           (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[4]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXRR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[3]->key, nodes[3]->value,
                                           nodes[3]->weight - 1,
                                           (Node <K, V> *) nodes[3]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 1,
                                          nodeXXRL, nodeXXRR);
    int weight = nodes[1]->weight;
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[2]->key, nodes[2]->value, weight,
                                         (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed), nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W1SYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW2Sym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                        bool fieldIsLeft) {
    Node <K, V> *nodeXXRL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[4]->key, nodes[4]->value, 0,
                                           (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[4]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXRR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[3]->key, nodes[3]->value,
                                           nodes[3]->weight - 1,
    ...
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[2]->key, nodes[2]->value, weight,
                                         (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed), nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W2SYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW3Sym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                        bool fieldIsLeft) {
    Node <K, V> *nodeXXRL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[4]->key, nodes[4]->value, 1,
                                           (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[5]->left.load(memory_order_relaxed));
    Node <K, V> *nodeXXRRR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 4), nodes[3]->key, nodes[3]->value,
                                            nodes[3]->weight - 1,
                                            (Node <K, V> *) nodes[3]->left.load(memory_order_relaxed),
                                            (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXRR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[1]->key, nodes[1]->value, 1,
                                           (Node <K, V> *) nodes[5]->right.load(memory_order_relaxed), nodeXXRRR);
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[5]->key, nodes[5]->value, 0,
                                          nodeXXRL, nodeXXRR);
    int weight = nodes[1]->weight;
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[2]->key, nodes[2]->value, weight,
                                         (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed), nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W3SYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW4Sym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                        bool fieldIsLeft) {
    Node <K, V> *nodeXXLR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[5]->key, nodes[5]->value, 1,
                                           (Node <K, V> *) nodes[5]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[5]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[2]->key, nodes[2]->value, 0,
                                          (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed), nodeXXLR);
    Node <K, V> *nodeXXRR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 4), nodes[3]->key, nodes[3]->value,
                                           nodes[3]->weight - 1,
                                           (Node <K, V> *) nodes[3]->left.load(memory_order_relaxed),
                                           (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[1]->key, nodes[1]->value, 1,
                                          (Node <K, V> *) nodes[4]->right.load(memory_order_relaxed), nodeXXRR);
    int weight = nodes[1]->weight;
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[4]->key, nodes[4]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W4SYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW5Sym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                        bool fieldIsLeft) {
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[4]->key, nodes[4]->value, 1,
                                          (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed),
                                          (Node <K, V> *) nodes[4]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXRR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[3]->key, nodes[3]->value,
                                           nodes[3]->weight - 1,
                                           ...
                                           (Node <K, V> *) nodes[3]->right.load(memory_order_relaxed));
    Node <K, V> *nodeXXR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 2), nodes[1]->key, nodes[1]->value, 1,
                                          (Node <K, V> *) nodes[2]->right.load(memory_order_relaxed), nodeXXRR);
    int weight = nodes[1]->weight;
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[2]->key, nodes[2]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W5SYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW6Sym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                        bool fieldIsLeft) {
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[2]->key, nodes[2]->value, 1,
                                          (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
                                          (Node <K, V> *) nodes[4]->left.load(memory_order_relaxed));
    Node <K, V> *nodeXXRR = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[3]->key, nodes[3]->value,
                                           nodes[3]->weight - 1,
    ...
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[4]->key, nodes[4]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W6SYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::doW7Sym(const int tid, Node <K, V> **nodes, void **llxResults,
                                                        bool fieldIsLeft) {
    Node <K, V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[2]->key, nodes[2]->value,
                                          nodes[2]->weight - 1,
                                          (Node <K, V> *) nodes[2]->left.load(memory_order_relaxed),
    ...
    Node <K, V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[1]->key, nodes[1]->value, weight,
                                         nodeXXL, nodeXXR);
    if (SCXAndEnterQuiescentState(tid, SCXRecord<K, V>::TYPE_W7SYM, nodes, llxResults,
                                  (fieldIsLeft ? &nodes[0]->left : &nodes[0]->right), nodeXX)) {
        return true;
    } else {
        return false;
...
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K, V, Compare, MasterRecordMgr>::tryRetireSCXRecord(const int tid, SCXRecord <K, V> *const otherSCX,
                                                                   Node <K, V> *const node) {
