...
public:
    ...
    struct rebind {
        typedef reclaimer_ebr_tree<_Tp1, Pool> other;
    };
    template<typename _Tp1, typename _Tp2>
    struct rebind2 {
        typedef reclaimer_ebr_tree<_Tp1, _Tp2> other;
    };
...
//        }
    ...
    reclaimer_ebr_tree(const int numProcesses, Pool *_pool, debugInfo *const _debug,
                       RecoveryMgr<void *> *const _recoveryMgr = NULL)
            : reclaimer_interface<T, Pool>(numProcesses, _pool, _debug, _recoveryMgr) {
        VERBOSE std::cout << "constructor reclaimer_ebr_tree helping=" << this->shouldHelp()
