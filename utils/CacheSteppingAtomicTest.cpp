...
atomic<double> *workloads;

void generate() {
    workloads = new atomic<double>[total];
    ...
    for (size_t i = 0; i < total; i++) {
        workloads[i].store(static_cast<double>(dis(engine)) / std::numeric_limits<uint32_t>::max());
    }
    ...
    for (size_t t = 0; t < degree; t++) {
        ...
        threads[t] = std::thread([](size_t tid, double &stat, long &time) {
            ...
            for (size_t r = 0; r < iteration; r++)
                for (size_t i = tid; i < total / degree; i += degree)
                    stat += workloads[((i + tid) * step) % total].load();
